...
function Header() {
  const btnIcon = 'cursor-pointer h-10 w-10  p-1 hover:p-0'
  ...
  return (
    <div className='h-16 w-screen md:hidden bg-white z-10 flex items-center justify-between gap-4 p-2 fixed top-0 border-b shadow-sm'>
        ...
        <div className='bg-gray-100 h-10 rounded-lg flex-1 flex items-center px-4 '>
            <FiSearch fontSize={20} className='text-gray-400' />
            <input type="text" className='outline-none border-none w-full h-full bg-transparent placeholder:text-lg placeholder:font-light px-2' placeholder='Search' />
        </div>
        <div 
        // onClick={()=> setActive({notifications: true})} 
        className='flex items-center gap-4 lg:mx-0 mx-auto transition-all duration-200 ease-out'>
          <HiOutlineHeart fontSize={30} className={btnIcon}/>
        </div>
    </div>
  )
}
